...
export class GitService {
  ...
  private initializeGitExtension() {
    try {
      this.gitExtension = {
        gitApi: vscode.extensions.getExtension('vscode.git')?.exports.model,
      };
  ...
  private async getStagedChangesViaCommand(
    ...
    try {
      ...
      throw new Error(
        '无法获取暂存区变更，请确保当前目录是Git仓库且有暂存的文件。'
      );
    }
  }
  ...
  async validateRepository(): Promise<{ isValid: boolean; error?: string }> {
    try {
      ...
      return {
        isValid: false,
        error:
          error instanceof Error ? error.message : '验证Git仓库时发生未知错误',
      };
    }
  }
}
