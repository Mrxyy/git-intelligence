...
export class ConfigService {
    ...
    static async getConfig(): Promise<GitCopilotConfig> {
        ...
        const language = commitlintConfig?.gitCopilot?.language || 
                        config.get<'zh-CN' | 'en-US'>('language', 'zh-CN');

        return {
            aiProvider: config.get<'openai' | 'anthropic' | 'google'>('aiProvider', 'openai'),
            apiKey: config.get<string>('apiKey', ''),
            modelName: config.get<string>('modelName', 'gpt-4o-mini'),
            promptTemplate: promptTemplate.replace(/\${lintStyles}/g, await this.generateLintPrompt()).trim(),
            language,
            commitlintRules: commitlintConfig?.rules
        };
    }

    static async updateConfig(key: keyof GitCopilotConfig, value: any): Promise<void> {
        const config = vscode.workspace.getConfiguration(this.CONFIG_SECTION);
    ...
    static async loadCommitlintConfig(): Promise<CommitlintConfig | null> {
        ...
        const commitlintConfigPath = path.join(workspacePath, 'commitlint.config.js');
    ...
    private static parseCommitlintConfig(content: string): CommitlintConfig | null {
        try {
            // 移除注释和清理内容
            ...
            const exportMatch = cleanContent.match(/module\.exports\s*=\s*({[\s\S]*})/);
            if (exportMatch) {
    ...
    static generatePromptFromCommitlintRules(rules: any, language: 'zh-CN' | 'en-US' = 'zh-CN'): string {
        if (!rules) {
            return '';
        }
    ...
    static async generateLintPrompt() {
        ...
        if (commitlintConfig?.gitCopilot?.promptTemplate) {
            ...
            const language = commitlintConfig?.gitCopilot?.language || 
                            config.get<'zh-CN' | 'en-US'>('language', 'zh-CN');
            lintTemplate = this.generatePromptFromCommitlintRules(commitlintConfig.rules, language);
        }
        return lintTemplate
    }
